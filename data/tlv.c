                         ...
                         lwm2m_data_t * dataP)
{
    ...
    for (i = 0 ; i < size && length != -1 ; i++)
    {
        ...
        LOG_ARG("dataP[%d].type: %d",i,dataP[i].type);
        ...
        case LWM2M_TYPE_CORE_LINK:
            ...
            LOG_ARG("prv_getLength loc 4  %d",length);
            break;
        ...
        case LWM2M_TYPE_INTEGER:
            ...
            LOG_ARG("prv_getLength loc 5  %d",length);	   
            break;
        ...
        case LWM2M_TYPE_FLOAT:
            {
                ...
                length += prv_getHeaderLength(dataP[i].id, data_len) + data_len;
                LOG_ARG("prv_getLength loc 6  %d",length);
            }
            break;
        ...
        case LWM2M_TYPE_BOOLEAN:
            ...
            LOG_ARG("prv_getLength loc 7  %d",length);	   
            break;
        ...
        case LWM2M_TYPE_OBJECT_LINK:
            ...
            LOG_ARG("prv_getLength loc 8  %d",length);	   
            break;

        default:
	   
	    LOG_ARG("prv_getLength loc 9  %d",0);
            length = -1;
            break;
        }
    }

    LOG_ARG("prv_getLength loc 10  %d",length);
    return length;
}
                  ...
                  uint8_t ** bufferP)
{
    ...
    *bufferP = NULL;
    ...
    if (length <= 0) {
       LOG_ARG("returning length %d", length);
       return length;
    }
    ...
    *bufferP = (uint8_t *)lwm2m_malloc(length);
    if (*bufferP == NULL) {
       LOG_ARG("returning bufferP == NULL  %d", 0);
       return 0;
    }
    ...
    for (i = 0 ; i < size && length != 0 ; i++)
    {
        ...
        case LWM2M_TYPE_MULTIPLE_RESOURCE:
            ...
            LOG_ARG("Calling MULTIPLE_RESOURCE %d",0);
        ...
        case LWM2M_TYPE_OBJECT_INSTANCE:
            {
                ...
                else
                {
                    ...
                    if (tmpLength > 0)
                    {
                        ...
                        index += tmpLength;
                        LOG_ARG("Calling lwm2m_free %d",0);
    ...
    if (length < 0)
    {
        LOG_ARG("Calling lwm2m_free %d",2);
        lwm2m_free(*bufferP);
        *bufferP = NULL;
    }
